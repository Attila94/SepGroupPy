...
# Share same spatial filter over input group dimension
...
def trans_filter(w, inds):
    ...
    w_indexed = w_indexed.view(w_indexed.size()[0], w_indexed.size()[1], inds.shape[0],
                               inds.shape[1], inds.shape[2], inds.shape[3])
    w_transformed = w_indexed.permute(0, 2, 1, 3, 4, 5)
    return w_transformed.contiguous()

def pre_trans_filter(dw,pw,inds):
    # Preconstruct full filter bank from spatial filter kernels and group weights.
    w = torch.mul(pw,dw[:,:,None,:,:])
...
class SplitGConv2D(nn.Module):
    ...
    def forward(self, input):
        ...
        # Input layer - nothing to decompose
        if self.input_stabilizer_size == 1:
        ...
        # Hidden layer
            ...
            if self.separable:
                ...
                # Reshape weights
                tw_pointwise = tw_pointwise.permute(2,0,1,3,4,5).contiguous()
                tw_shape = (self.out_channels * self.output_stabilizer_size * self.in_channels,
                            self.input_stabilizer_size, 1, 1)
                tw = tw_pointwise.view(tw_shape)
                # Convolve
                y = F.conv2d(input, weight=tw, bias=None, groups=self.in_channels)
                y = y.view(input_shape[0], self.in_channels, self.out_channels, self.input_stabilizer_size, input_shape[-2], input_shape[-1])
                y = y.permute(0,2,3,1,4,5).contiguous()

                # Spatial convolution
                tw_depthwise = trans_filter_d(self.depthwise_weight[:,None,:,:,:],
                                              self.input_stabilizer_size)
                # Reshape weights
                tw_shape = (self.out_channels * self.output_stabilizer_size,
                            self.in_channels, self.ksize, self.ksize)
                tw = tw_depthwise.view(tw_shape)
                # Reshape intermediate feature map
                y = y.view((input_shape[0], self.out_channels * self.output_stabilizer_size * self.in_channels, input_shape[-2], input_shape[-1]))
                # Convolve
            ...
            # Non-separable convolution (preconstructed filter)
            else:
...
class gP4ConvZ2(SplitGConv2D):
    # Same as regular P4ConvZ2
    def __init__(self, *args, **kwargs):
        super(gP4ConvZ2, self).__init__(input_stabilizer_size=1, output_stabilizer_size=4, *args, **kwargs)


class gP4ConvP4(SplitGConv2D):
    def __init__(self, *args, **kwargs):
        super(gP4ConvP4, self).__init__(input_stabilizer_size=4, output_stabilizer_size=4, *args, **kwargs)


class gP4MConvZ2(SplitGConv2D):
    # Same as regular P4MConvZ2
    def __init__(self, *args, **kwargs):
        super(gP4MConvZ2, self).__init__(input_stabilizer_size=1, output_stabilizer_size=8, *args, **kwargs)


class gP4MConvP4M(SplitGConv2D):
    def __init__(self, *args, **kwargs):
        super(gP4MConvP4M, self).__init__(input_stabilizer_size=8, output_stabilizer_size=8, *args, **kwargs)
